...
import { UserCurrentNetflixUser } from "./use-current-user"; 
  ...
  persist<UseAddFilmMyList>(
    (set, get) => ({
      ...
      addLovedFilm: (data: Movie) => {
        const { currentUser } = UserCurrentNetflixUser.getState(); // Get the current user
      ...
      removeLovedItem: (id: string) => {
        const { currentUser } = UserCurrentNetflixUser.getState();
